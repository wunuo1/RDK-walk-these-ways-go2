...
from go2_gym_deploy.lcm_types.example_t import example_t
from go2_gym_deploy.lcm_types.ground_type import ground_type
# 不调用相机 !!!
...
class StateEstimator:
    def __init__(self, lc, use_cameras=False): # defaul use_cameras=True
        ...
        self.control_command_subscription = self.lc.subscribe("cmd_vel", self._control_command_cb)
        self.ground_type_subscription = self.lc.subscribe("ground_type", self._ground_type_cb)
# --------------------------------------------------------------
        # if use_cameras:
    ...
    def get_command(self):
        ...
        if self.command_control == False:
            self.cmd_x = 1 * self.left_stick[1]
            self.cmd_yaw = -1 * self.right_stick[0]
            self.cmd_y = 0.  # -1 * self.left_stick[0]

            # joystick commands
            if MODE_LEFT == "body_height":
                cmd_height = 0.3 * self.left_stick[0]
            elif MODE_LEFT == "lat_vel":
                self.cmd_y = 0.6 * self.left_stick[0]
        ...
        #grond_types
        if self.ground_type == 0:  #brick
            cmd_x = self.cmd_x - 0.02
            cmd_y = self.cmd_y * 0.3 + 0.05
            cmd_yaw = self.cmd_yaw + 0.02
            cmd_footswing = 0.23
        elif self.ground_type == 3:  #stone
            cmd_x = self.cmd_x - 0.05
            cmd_y = self.cmd_y * 1.2 - 0.1
            cmd_yaw = self.cmd_yaw - 0.025
            cmd_height = -0.15
            cmd_freq = 2.0
        else: #default and grass
            cmd_x = self.cmd_x - 0.04
            cmd_y = self.cmd_y - 0.08
            cmd_yaw = self.cmd_yaw + 0.025
               
        print("\ncmd_x: ", cmd_x)
        print("cmd_y: ", cmd_y)
        print("cmd_yaw: ", cmd_yaw)
        print("cmd_height: ", cmd_height)
        print("cmd_freq: ", cmd_freq)
        print("cmd_footswing: ", cmd_footswing)
        print("model: ", self.mode)
        print("command_control: ",self.command_control)
        print("ground_type: ", self.ground_type)
    ...
    def _control_command_cb(self, channel, data):
        if self.command_control == True:
            msg = example_t.decode(data)
            #print("msg_y: ",msg.vel[1])
            self.cmd_x = msg.vel[0] / 0.5 * 0.1
            self.cmd_y = msg.vel[1] / 0.3 * 0.1 * 0.8
            self.cmd_yaw = msg.vel[2] / 1.2 * 0.1

            # self.cmd_x = msg.vel[0]
            # self.cmd_y = msg.vel[1]
            # self.cmd_yaw = msg.vel[2]

            # print(f"Received on {channel}: x={msg.vel[0]}, y={msg.vel[1]}, theta={msg.vel[2]}")

    def _ground_type_cb(self, channel, data):
        msg = ground_type.decode(data)
        self.ground_type = msg.type
